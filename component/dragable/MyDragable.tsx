...
export const DraggableGrid = function<DataType extends IBaseItemType>(
  ...
  const assessGridSize = (event: IOnLayoutEvent) => {
    if (!hadInitBlockSize) {
      let blockWidth = event.nativeEvent.layout.width / 2
  ...
  function initBlockPositions() {
    items.forEach((item, index) => {
      blockPositions[index] = getBlockPositionByOrder(item?.itemData,index)
    })
  }

  function getBlockPositionByOrder(item: DataType,order: number) {
    // if (blockPositions[order]) {
    //   return blockPositions[order]
    // }
        
    const columnOnRow = order % 2.3
    const y = item.radius > 100 ? item.radius * Math.floor(order / 2.3): 120 * Math.floor(order / 2.3)
    const x = item.radius > 100 ? item.radius * columnOnRow : 120 * columnOnRow
    return {
      x,
      y,
    }
  ...
  function resetGridHeight() {
    const rowCount = Math.ceil(props.data.length / 2.3)
  ...
  function getBlockStyle(itemIndex: number) {
    return [
      ...
      hadInitBlockSize && {
        width: items[itemIndex].itemData.radius,
        height: items[itemIndex].itemData.radius,
        position: 'absolute',
  ...
  function addItem(item: DataType, index: number) {
    blockPositions.push(getBlockPositionByOrder(item,items.length))
    orderMap[item.key] = {
      order: index,
    }
    ...
    items.push({
      key: item.key,
      itemData: item,
      currentPosition: new Animated.ValueXY(getBlockPositionByOrder(item,index)),
    })
  }
